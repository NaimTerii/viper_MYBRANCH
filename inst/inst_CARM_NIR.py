...
def driftcorr(tag, obsnames, orders, chunks, tellshift_option=False):
            ...
            if tellshift_option:
                ...
                ''' 
                Mix drift_mes with weighted mean tellshift (1 drift value per file)
                Computes self.tellshift_wmean (1 value per file)
                Brings self.tellshift_wmean to same median (and same sign) as self.drift_mes (sign sucb that it needs to be subtracted from RVs?)
                Computes the self.drift_mix array (mix of drift_mes and tellshift_wmean)
                '''
                ...
                ## ---> but then the fitted tellshift still has that effect? so idk (we would only remove this offset for drift correction but not for actually shifting the telluric lines)
                ...
                ## if only 1 order could be analyzed,then tellshift_wmean = the tellshift for that order, and e_tellshift_wmean = 0. 
                ...
                ### Change the sign of tellshift to match the sign of drift_mes
                tellshift_wmean *= -1
            ...
            * Can not write the drift because then it doesnt work with vpr.py :
                LINE 144 of vpr.py :
                        self.full_rv, self.full_e_rv = mat.T[4:-1:2], mat.T[5::2]
                which i would need to CHANGE depending on whether the drift is written into file or not
                
            if drift is not printed in the file then should i just make it a .rvo.dat file?

            '''
            # Currently this re-writes the .rvo.dat file. Could try writing in a separate '.rvc' file? 
            rvcfile = open(tag+'.rvc.dat', 'w')
            #print('BJD RV e_RV BERV drift e_drift', *map("rv{0} e_rv{0}".format, colnums), 'filename', file=rvcfile)
